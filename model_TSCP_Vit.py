...
class TSCP_Vit_block(nn.Module):
    def __init__(self, squeezed_patch_size,embed_channel, num_heads, ffn_channel):
        ...
        self.ffn = nn.Sequential(
            nn.Conv2d(in_channels=self.embed_channel, out_channels=ffn_channel, kernel_size=5, stride=1, padding=2),
            NRG_GELU(),
            nn.Conv2d(in_channels=ffn_channel, out_channels=self.embed_channel, kernel_size=5, stride=1, padding=2),
        )
...
class TSCP_Vit(nn.Module):
    def __init__(self, img_size=(224,224), patch_size=(16,16), squeeze_rate=(4,4),in_channel=3, embed_channel=72, num_heads=12, num_layers=12, ffn_channel=288,num_classes=1000):
        ...
        for _ in range(num_layers):
            self.TSCP_Vit_block_list.append(TSCP_Vit_block(self.squeezed_patch_size,self.embed_channel, num_heads, ffn_channel))
